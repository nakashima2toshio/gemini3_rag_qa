...
本アプリケーションは、RAG（Retrieval-Augmented Generation）システムのための統合ツールです。LLMとしてGoogle Gemini APIを基本として利用します。以下の機能を提供します：

-   **データセット取得**: HuggingFaceまたはローカルファイルからデータをダウンロード・前処理
-   **Q/Aペア自動生成**: Gemini API (`gemini-2.0-flash`等) を使用してドキュメントからQ&Aペアを自動生成
-   **ベクトルDB登録**: 生成したQ&AペアをQdrantベクトルデータベースに登録（EmbeddingにはGemini API `gemini-embedding-001`を使用）
-   **セマンティック検索**: 登録データに対する意味検索とAI応答生成（LLMはGemini APIを使用）

### 1.2 動作要件・依存サービス

| サービス           | 用途                           | デフォルトURL         |
| ------------------ | ------------------------------ | --------------------- |
| **Google Gemini API** | Q/A生成・埋め込み生成          | API Key必須           |
...
![データセット選択.png](assets/データセット選択.png)
...
| モデル         | Q/A生成に使用するモデル    | **gemini-2.0-flash** |
...
1.  CSVファイル読み込み
2.  コレクション作成/再作成
3.  埋め込みベクトル生成（**Gemini `gemini-embedding-001` (3072次元)**）
4.  Qdrantポイント構築
5.  Qdrantアップサート
...
最高スコアの結果に基づき、**Gemini (`gemini-2.0-flash`)** によるAI応答が生成されます。
...
| `API key not found`    | Google Gemini API Keyが設定されていない | `.env`ファイルに`GOOGLE_API_KEY`を設定 |
...
# Celeryワーカー起動
./start_celery.sh restart -w 8 # Gemini APIのレート制限に合わせてワーカー数を調整
...
# コマンドラインでQdrant登録
# services/qdrant_service.py にてGemini Embeddingを使用
